...
def open_modal_with_favs():
    ...
    return (
        ...
        gr.update(visible=True),   # modal_footer
        st.isoformat(),            # start_dt (ISO 문자열로 전달)
        en.isoformat(),            # end_dt (ISO 문자열로 전달)
        "",                        # msg_output
        gr.update(choices=favs, value=None)  # favorites_dropdown
    )
...
def close_modal_all():
    return (
        ...
        gr.update(visible=False),  # modal_footer
    )
...
with gr.Blocks(css=CSS, title="Oseyo") as demo:
    ...
    with gr.Column(visible=False, elem_classes=["modal-sheet"]) as modal_sheet:
        ...
        start_dt = gr.DateTime(label="📅 시작 일시 (클릭하여 선택)", include_time=True, interactive=True, value=None)
        end_dt = gr.DateTime(label="⏰ 종료 일시 (클릭하여 선택)", include_time=True, interactive=True, value=None)
