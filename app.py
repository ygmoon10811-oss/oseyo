...
/* ✅ 이벤트명 입력창이 길어져도 다른 요소(이미지 업로드 등)를 '가리지' 않게: 높이 제한 + 내부 스크롤 */
#event_title textarea{
  max-height: 120px !important;   /* 필요시 100~160px 사이로 조절 */
  overflow-y: auto !important;
  resize: none !important;        /* 사용자가 드래그로 과도하게 늘리는 것도 방지 */
  line-height: 1.4 !important;
}

/* 혹시 모달 레이아웃에서 겹침이 생기는 환경 방지 */
...
                t_in = gr.Textbox(
                    ...
                    elem_id="event_title",
                    lines=2,        # 기본 높이
                    max_lines=4     # (버전별로 동작이 조금 달라도) 과도한 확장을 막는 안전장치
                )
