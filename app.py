...
# 1. 설정 및 환경 변수 (Render 환경 변수에서 읽어옴)
...
# 2. DB 설정 (Render 디스크 권한 대응)
def get_db_path():
    # Render의 무료 티어는 재배포 시 파일이 초기화되므로 /tmp를 쓰거나 
    # 루트에 생성하되, 데이터 보존이 필요하면 유료 Disk를 연결해야 합니다.
    return "oseyo.db"
...
def db_insert_space(space: dict):
    with db_conn() as con:
        con.execute("""
        INSERT INTO spaces (id, title, photo_b64, start_iso, end_iso, address, lat, lng, created_at)
        VALUES (?,?,?,?,?,?,?,?,?)
        """, (space["id"], space["title"], space["photo_b64"], space["start_iso"], space["end_iso"], 
              space["address"], space["lat"], space["lng"], now_kst().isoformat()))
        con.commit()
...
def active_spaces():
    ...
    for r in rows:
        try:
            ...
            if st <= t <= en:
                out.append({"id":r[0], "title":r[1], "photo_b64":r[2], "start_iso":r[3], "end_iso":r[4], "address":r[5], "lat":r[6], "lng":r[7]})
        except: continue
    return out
...
def kakao_keyword_search(q):
    if not q or not KAKAO_REST_API_KEY:
        return [], "⚠️ API 키가 설정되지 않았습니다. (Render 환경 변수 확인)"
    try:
...
.card{background:white; border-radius:20px; padding:15px; margin-bottom:12px; border:1px solid #E5E7EB; position:relative;}
.thumb{width:75px; height:75px; object-fit:cover; border-radius:15px;}
"""
...
with gr.Blocks(css=CSS, title="오세요") as demo:
    ...
    with gr.Tabs():
        with gr.Tab("목록 보기"):
            def get_list():
                ...
                for s in items:
                    ...
                    res += f"<div class='card'><div style='display:flex; justify-content:space-between;'><div><div style='font-weight:800; font-size:18px;'>{s['title']}</div><div style='color:#666; font-size:13px;'>{s['address']}</div></div>{img}</div><a href='/delete/{s['id']}' style='color:red; font-size:12px;'>닫기</a></div>"
                return res
        ...
        with gr.Tab("지도 보기"):
            def get_map():
                ...
                return f"""<div style='width:100%; height:450px; border-radius:20px; overflow:hidden;' id='map'></div>
                <script>
            ...
            map_out = gr.HTML(get_map)
            gr.Button("🔄 지도 새로고침").click(get_map, outputs=map_out)
    ...
    with gr.Column(visible=False, elem_classes=["modal-sheet"]) as modal_sheet:
        ...
        title_in = gr.Textbox(label="무엇을 하나요?", placeholder="활동 이름을 적어주세요")
        img_in = gr.Image(label="이미지", type="numpy", height=150)
        with gr.Row():
            start_in = gr.DateTime(label="시작 일시", value=now_kst)
            end_in = gr.DateTime(label="종료 일시", value=lambda: now_kst()+timedelta(hours=2))
        with gr.Row():
            q_in = gr.Textbox(label="장소 검색", placeholder="장소명 입력", scale=4)
            s_btn = gr.Button("🔍", scale=1)
        drop_in = gr.Dropdown(label="결과 선택", choices=[])
        msg_in = gr.Markdown("")
        with gr.Row():
            c_btn = gr.Button("취소")
    ...
    def save_event(title, img, s, e, p_js):
        if not title or p_js == "{}": return "⚠️ 필수 항목을 확인하세요.", get_list(), get_map(), gr.update(visible=True)
        p = json.loads(p_js)
        b64 = ""
        if img is not None:
            buf = io.BytesIO(); Image.fromarray(img).save(buf, format="JPEG"); b64 = base64.b64encode(buf.getvalue()).decode()
        db_insert_space({"id":uuid.uuid4().hex[:8], "title":title, "photo_b64":b64, "start_iso":s, "end_iso":e, "address":p["place"], "lat":p["lat"], "lng":p["lng"]})
        return "✅ 완료!", get_list(), get_map(), gr.update(visible=False)

    ok_btn.click(save_event, [title_in, img_in, start_in, end_in, place_json], [msg_in, list_out, map_out, modal_sheet]).then(
        lambda m: [gr.update(visible=False)]*2 if "✅" in m else [gr.update(visible=True)]*2, msg_in, [modal_overlay, modal_sheet])

# 6. FastAPI & Render Port 설정
app = FastAPI()
@app.get("/")
def go_app(): return RedirectResponse("/app")
@app.get("/delete/{sid}")
def del_sp(sid):
...
# 카카오맵 SDK 주입
HEAD = f"<script src='//dapi.kakao.com/v2/maps/sdk.js?appkey={KAKAO_JAVASCRIPT_KEY}'></script>"
app = gr.mount_gradio_app(app, demo, path="/app", head=HEAD)

if __name__ == "__main__":
    import uvicorn
    # Render가 제공하는 $PORT 환경변수를 사용해야 502 에러가 나지 않습니다.
