...
.fav-grid { display: grid !important; grid-template-columns: 1fr 1fr !important; gap: 6px !important; }
...
def get_map_html():
    ...
    return f'<iframe src="/kakao_map?ts={ts}" class="map-frame"></iframe>'
...
with gr.Blocks(css=CSS) as demo:
    ...
    with gr.Column(visible=False, elem_classes=["modal-sheet"]) as modal:
        ...
        with gr.Column(elem_classes=["modal-scroll"]):
            ...
            # 주소 검색 영역
            loc_in = gr.Textbox(label="📍 장소 검색", placeholder="장소명 입력")
...
@app.get("/kakao_map")
def kakao_map():
    with db_conn() as con: rows = con.execute("SELECT title, lat, lng, address, start_iso FROM spaces").fetchall()
    pts = [{"title": r[0], "lat": r[1], "lng": r[2], "addr": r[3], "time": r[4]} for r in rows]
    center = pts[0] if pts else {"lat": 36.019, "lng": 129.343}
    html = f"""
    <html><head><script src="//dapi.kakao.com/v2/maps/sdk.js?appkey={KAKAO_JAVASCRIPT_KEY}"></script></head>
    <body style="margin:0;"><div id="map" style="width:100%;height:100%;"></div><script>
    var map = new kakao.maps.Map(document.getElementById('map'), {{center: new kakao.maps.LatLng({center['lat']}, {center['lng']}), level: 7}});
    var pts = {json.dumps(pts)};
    pts.forEach(function(p) {{
        var m = new kakao.maps.Marker({{map: map, position: new kakao.maps.LatLng(p.lat, p.lng)}});
        var iw = new kakao.maps.InfoWindow({{content: '<div style="padding:5px;font-size:12px;"><b>'+p.title+'</b><br>'+p.addr+'</div>'}});
