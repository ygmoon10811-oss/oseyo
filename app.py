...
/* ✅ 이벤트명 길어져도 아래 영역 가리지 않도록: 높이 제한 + 내부 스크롤 */
...
/* ✅ 즐겨찾기 갱신/리렌더링 때 이미지 업로드 영역이 접히는 현상 방지 */
#event_photo {
...
# -----------------------------
# 4) Gradio UI (로그인 후 보여줄 '그 화면' 그대로)
# -----------------------------
...
with gr.Blocks(css=CSS, title="오세요") as demo:
    ...
    with gr.Tabs():
        ...
        with gr.Tab("지도"):
            gr.HTML(
                '<iframe src="/map" style="width:100%;height:70vh;border:none;border-radius:16px;"></iframe>'
            )
...
async def auth_guard(request: Request, call_next):
    ...
    # 누구나 접근 가능
    if path in ("/health", "/login", "/signup", "/logout"):
        return await call_next(request)

    # Gradio 정적파일/웹소켓 등 (그라디오 내부 라우팅은 통과)
    if path.startswith(("/assets", "/static", "/favicon.ico")):
        return await call_next(request)

    # 보호 대상: 루트, /app, /map 모두 로그인 필요
    if path == "/" or path.startswith("/app") or path.startswith("/map"):
...
@app.get("/")
def root():
    # 로그인 됐으면 /app로 보내고, 아니면 middleware가 /login으로 보냄
...
<head>
  ...
  <style>
    ...
    .wrap { max-width:420px; margin:48px auto; background:white; border:1px solid #eee; border-radius:16px; padding:20px; }
    h2 { margin:0 0 14px 0; }
    input { width:100%; padding:12px; margin:8px 0; border:1px solid #ddd; border-radius:12px; }
    button { width:100%; padding:12px; border:none; border-radius:12px; background:#ff6b00; color:white; font-weight:800; }
...
<head>
  ...
  <style>
    ...
    h2 { margin:0 0 14px 0; }
    input { width:100%; padding:12px; margin:8px 0; border:1px solid #ddd; border-radius:12px; }
    button { width:100%; padding:12px; border:none; border-radius:12px; background:#111; color:white; font-weight:800; }
    a { color:#ff6b00; text-decoration:none; }
    .muted { color:#777; font-size:13px; margin-top:10px; }
  </style>
</head>
